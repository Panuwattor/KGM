...
<div class="container faq-wrap" style="max-width:860px;" x-data="{ open: null }">
    ...
    {{-- สินค้าและไซซ์ --}}
    <div class="faq-group">
        <h2 class="faq-group-title"><i class="bi bi-rulers"></i> สินค้าและไซซ์</h2>

        <div class="faq-item" :class="{ active: open === 's1' }">
            <button class="faq-q" @click="open = open === 's1' ? null : 's1'">
                <span>จะรู้ได้อย่างไรว่าไซซ์ไหนพอดีกับตัว?</span>
                <i class="bi bi-chevron-down faq-arrow"></i>
            </button>
            <div class="faq-a" x-show="open === 's1'" x-collapse>
                <p>แนะนำให้ดู<a href="{{ route('size-guide') }}">คู่มือวัดไซซ์</a>ของเรา มีขั้นตอนการวัดรอบอก รอบเอว ความยาวแขน และอื่น ๆ อย่างละเอียด หากยังไม่แน่ใจสามารถติดต่อทีมงานเพื่อรับคำแนะนำเพิ่มเติม</p>
            </div>
        </div>
        ...
        <div class="faq-item" :class="{ active: open === 's3' }">
            ...
            <div class="faq-a" x-show="open === 's3'" x-collapse>
                <p>สินค้าสำเร็จรูปในเว็บไซต์มีสต็อกพร้อมส่ง โดยจะแสดงจำนวนคงเหลือในหน้าสินค้า หากไซซ์หรือสีที่ต้องการหมด สามารถสอบถามเพื่อจองรุ่นผลิตใหม่ได้</p>
            </div>
        </div>
    </div>
