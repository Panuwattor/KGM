@extends('layouts.app')
@section('title', 'สินค้าโปรโมชั่น - ลดราคาพิเศษ')
    ...
    <div class="shop-layout">
            ...
            <div x-show="open" x-transition:enter="transition-all duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-all duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="block-on-desktop">
                <div class="filter-card">
                    ...
                    <div class="type-pills">
                        <a href="{{ route('promotions') }}" class="type-pill {{ !$selectedType ? 'active' : '' }}">
                            <i class="bi bi-grid"></i> ทั้งหมด
                        </a>
                        @foreach($productTypes as $pt)
                        <a href="{{ route('promotions') }}?type={{ $pt->slug }}" class="type-pill {{ $selectedType?->id === $pt->id ? 'active' : '' }}">
                            {{ $pt->name }}
                        </a>
                        @endforeach
                    </div>
                ...
                <div class="filter-card">
                    ...
                    <a href="{{ route('promotions') }}" class="cat-link {{ !request('category') ? 'active' : '' }}"><i class="bi bi-grid"></i> โปรโมชั่นทั้งหมด</a>
                    @foreach($categories as $cat)
                    <a href="{{ route('promotions') }}?category={{ $cat->slug }}" class="cat-link {{ request('category') === $cat->slug ? 'active' : '' }}">
                        <i class="bi bi-chevron-right" style="font-size:11px;"></i> {{ $cat->name }}
                    </a>
                    @if($cat->children->isNotEmpty())
                        @foreach($cat->children as $child)
                        <a href="{{ route('promotions') }}?category={{ $child->slug }}" class="cat-link cat-sub {{ request('category') === $child->slug ? 'active' : '' }}">
                            {{ $child->name }}
                        </a>
                        @endforeach
                    @endif
        ...
        <div>
            ...
            <div class="promo-banner">
                <h2>
                    <i class="bi bi-tag-fill" style="font-size:28px;"></i>
                    สินค้าโปรโมชั่น - ลดราคาพิเศษ
                </h2>
                <p>รวมสินค้าลดราคาพิเศษทั้งหมด ราคาดีที่สุด อัปเดตทุกวัน</p>
            </div>
            ...
            <div class="filter-bar">
                <form method="GET" action="{{ route('promotions') }}">
                    ...
                    <div class="filter-actions">
                        ...
                        <a href="{{ route('promotions') }}" class="btn btn-light"><i class="bi bi-x"></i> ล้าง</a>
                    </div>
                </form>
            </div>
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
                <div>
                    <h1 style="font-size:22px;font-weight:800;color:var(--kgm-green-800);">
                        <i class="bi bi-tag-fill" style="font-size:18px;margin-right:6px;color:#e63946;"></i>
                        @if($selectedType)
                            {{ $selectedType->name }} - โปรโมชั่น
                        @else
                            สินค้าโปรโมชั่นทั้งหมด
                        @endif
                    </h1>
                    <p style="font-size:13px;color:#888;">พบ {{ $products->total() }} รายการ</p>
                </div>
                @if($selectedType)
                <a href="{{ route('promotions') }}" class="btn btn-light" style="font-size:13px;"><i class="bi bi-x"></i> ล้างประเภท</a>
                @endif
            </div>
            ...
            <div style="text-align:center;padding:80px 0;color:#aaa;">
                <i class="bi bi-tag" style="font-size:64px;display:block;margin-bottom:16px;"></i>
                <div style="font-size:18px;font-weight:600;">ไม่พบสินค้าโปรโมชั่น</div>
                <p>ขณะนี้ยังไม่มีสินค้าลดราคาในหมวดหมู่นี้</p>
            </div>
            @else
