...
@section('title', 'แก้ไขข่าวสำคัญ')
@section('content')
<div class="page-header">
    <div class="page-title">แก้ไขข่าวสำคัญ</div>
    <a href="{{ route('admin.news-tickers.index') }}" class="btn btn-light"><i class="bi bi-arrow-left"></i> กลับ</a>
</div>
<div style="max-width:700px;">
<form method="POST" action="{{ route('admin.news-tickers.update', $newsTicker) }}">
    @csrf
    @method('PUT')
    <div class="form-card">
        ...
        <div class="form-group">
            ...
            <textarea name="plain_text" class="form-control" rows="3" placeholder="เช่น: ยินดีต้อนรับสู่เว็บไซต์กิจเจริญการ์เมนท์">{{ old('plain_text', $newsTicker->plain_text) }}</textarea>
        ...
        <div class="form-group">
            ...
            <input type="text" name="link_text" class="form-control" value="{{ old('link_text', $newsTicker->link_text) }}" placeholder="เช่น: สมัครเลย, คลิกที่นี่">
            @error('link_text')<div class="text-danger">{{ $message }}</div>@enderror
        </div>

        <div class="form-group">
            <label class="form-label">ลิงก์ (URL)</label>
            <input type="url" name="link_url" class="form-control" value="{{ old('link_url', $newsTicker->link_url) }}" placeholder="https://...">
        ...
        <div class="form-group">
            ...
            <input type="number" name="order" class="form-control" value="{{ old('order', $newsTicker->order) }}" min="0">
            @error('order')<div class="text-danger">{{ $message }}</div>@enderror
            <small style="color:#888;">เลขน้อยจะแสดงก่อน</small>
        </div>

        <div class="form-check">
            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $newsTicker->is_active) ? 'checked' : '' }}>
